...
import { submitCardSubscription } from "@/lib/subscriptions/checkout";
import { checkoutFailure } from "@/lib/subscriptions/checkout-diagnostics";
import { createClient } from "@/lib/supabase/server";
import { subscriptionCardSchema } from "@/lib/validation/subscription-card";
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) return NextResponse.json({ ok: false, message: "Revisá el correo y la autorización del débito." }, { status: 422 });
  ...
  try {
    const result = await submitCardSubscription(supabase, profile.id, parsed.data);
    return NextResponse.json({ ok: true, ...result, message: result.status === "ACTIVE" ? "Tu suscripción está activa. Confirmamos el pago con Mercado Pago." : "Recibimos la autorización del débito. Tu suscripción se activará cuando Mercado Pago confirme el pago." });
  } catch (error) {
    checkoutFailure("provider_create", "operation_failed", error);
    return NextResponse.json({ ok: false, message: "No pudimos confirmar la autorización. Revisá los datos o consultanos para recuperar el intento sin duplicarlo." }, { status: 409 });
  }
}
