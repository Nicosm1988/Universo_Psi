...
import { PayPalCheckout } from "@/components/subscriptions/paypal-checkout";
import { getPayPalCheckoutContext } from "@/lib/subscriptions/paypal";
...
export default async function SubscriptionCheckoutPage({ searchParams }: { searchParams: Promise<{ subscriptionId?: string; checkout?: string; error?: string; provider?: string }> }) {
  ...
  const paypal = id.success && profile && !error ? await getPayPalCheckoutContext(supabase, id.data, profile.id) : null;
  const usePayPal = query.provider === "paypal" || (query.provider !== "mercadopago" && (paypal?.provider === "paypal" || !context));
  if ((!context && !paypal) || !id.success) return <section className="rounded-3xl border border-line bg-paper p-7">
  ...
  const embedded = context && query.checkout === "card" && context.publicKey && context.snapshot.payment_model === "RECURRING";
  const nonce = embedded ? (await headers()).get("x-nonce") ?? undefined : undefined;
  const price = context ? new Intl.NumberFormat("es-AR", { style: "currency", currency: context.snapshot.currency }).format(context.snapshot.price_amount) : "";
  return <section className="mx-auto max-w-2xl rounded-3xl border border-line bg-paper p-6 sm:p-9">
    ...
    <h1 className="mt-3 text-3xl font-semibold">{(context?.snapshot.name ?? paypal?.snapshot.name) ?? "Plan profesional"}</h1>
    <nav aria-label="Medio de pago" className="mt-5 flex gap-5">
      <Link aria-current={!usePayPal ? "page" : undefined} href={{ pathname: "/dashboard/suscripcion/pagar", query: { subscriptionId: id.data, provider: "mercadopago" } }} className="inline-flex min-h-11 items-center font-semibold text-senda underline">Mercado Pago</Link>
      <Link aria-current={usePayPal ? "page" : undefined} href={{ pathname: "/dashboard/suscripcion/pagar", query: { subscriptionId: id.data, provider: "paypal" } }} className="inline-flex min-h-11 items-center font-semibold text-senda underline">PayPal</Link>
    </nav>
    {usePayPal ? <PayPalCheckout subscriptionId={id.data} available={paypal?.available ?? false} price={paypal?.price ? `${paypal.price.amount} ${paypal.price.currency}${paypal.snapshot.payment_model === "RECURRING" ? " por mes" : " por 12 meses"}` : null} /> : context ? <>
    ...
    </> : <p className="mt-4">Este pago ya está iniciado con PayPal. Continuá con ese medio para evitar duplicados.</p>}
  </section>;
}
