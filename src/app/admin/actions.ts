...
/** Cambia el estado de un mensaje del formulario de contacto. */
export async function resolveSupportRequestAction(formData: FormData) {
  const parsed = supportResolutionSchema.safeParse({
    requestId: formString(formData, "requestId"),
    status: formString(formData, "status"),
    notes: formString(formData, "notes"),
  });
  if (!parsed.success) {
    redirect("/admin?error=support-invalid" as Route);
  }
  ...
  if (error) {
    ...
    redirect("/admin?error=support-failed" as Route);
  }
  revalidatePath("/admin");
  redirect("/admin?notice=support-resolved" as Route);
}
