...
test("contacto registra un reporte y deriva los derechos de datos a /solicitudes", async ({ page }) => {
  await page.goto("/contacto?motivo=REPORTE");

  await expect(page.getByRole("heading", { level: 1 })).toContainText("Escribinos");
  await expect(page.getByRole("combobox", { name: "Motivo" })).toHaveValue("REPORTE");
  await expect(
    page.getByRole("link", { name: "formulario de baja, arrepentimiento y privacidad" }),
  ...
  await page
    ...
    .fill("Encontré un perfil que publica la matrícula de otra persona.");
