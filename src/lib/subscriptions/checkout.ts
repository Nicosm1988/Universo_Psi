...
export async function createCheckoutRedirectUrl(
  ...
  try {
    ...
    const { data: row, error: readError } = await supabase.from("subscriptions")
      .select("plan_snapshot,status,provider,provider_account,provider_subscription_id,provider_plan_id")
    ...
    if (!row || row.provider === "paypal") return checkoutFailure(stage, "missing_subscription");
...
export async function getHostedCheckoutContext(supabase: SupabaseClient, subscriptionId: string, profileId: string, allowCanceledRecovery = false) {
  ...
  if (error || !row || row.provider === "paypal" || (row.status !== "PENDING_PAYMENT" && !(allowCanceledRecovery && row.status === "CANCELED"))) return null;
...
export async function createHostedCheckoutRedirectUrl(supabase: SupabaseClient,
  ...
  if (context) return `/dashboard/suscripcion/pagar?subscriptionId=${params.subscriptionId}`;
  const { getPayPalCheckoutContext } = await import("@/lib/subscriptions/paypal");
  const paypal = await getPayPalCheckoutContext(supabase, params.subscriptionId, params.profileId);
  return paypal ? `/dashboard/suscripcion/pagar?subscriptionId=${params.subscriptionId}&provider=paypal` : null;
}
